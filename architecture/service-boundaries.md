...
topics: ["omniclaude", "service-boundaries", "omniintelligence"]
refs: []
---
...
**Last Updated**: 2026-08-25 (corrected during KB migration — prior banner read 2025-11-05, stale against the file's actual last edit)
**Version**: 1.0.0
...
**Components**:
- `src/omniclaude/lib/core/manifest_injector.py` - Dynamic manifest generation via Kafka (moved from `agents/lib/`)
- `src/omniclaude/lib/core/routing_event_client.py` - Event-based routing client (moved from `agents/lib/`)
- `agent_execution_logger.py` / `agent_history_browser.py` - removed; no successor module found in the current tree
...
**See**: this repo's SPDX/secrets handling in `CLAUDE.md`; a standalone `SECURITY_KEY_ROTATION.md` does not exist in this repo
...
# agents/lib/agent_history_browser.py has been removed from the tree;
# query Qdrant directly via the collection endpoints above instead.
```
...
(A "Manifest Intelligence" architecture doc and a standalone `SECURITY_KEY_ROTATION.md` were referenced here previously; neither exists in the current tree — removed rather than left as dead links.)
