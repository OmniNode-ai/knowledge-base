...
topics: ["omniclaude", "compliance", "hooks"]
refs: []
---
...
| File | Role |
|------|------|
| `src/omniclaude/hooks/lib/compliance_result_subscriber.py` | Kafka consumer, violation → advisory transform, persistence (moved from `plugins/onex/hooks/lib/`) |
...
| `delegation_orchestrator.py` | Removed from the tree — see `docs/architecture/DELEGATION_ARCHITECTURE.md`; this row described the now-removed UserPromptSubmit delegation bridge |
