...
status: accepted
date: "2026-06-24"
title: "Hook Data Flow Architecture"
topics: ["omniclaude", "hooks", "data-flow"]
refs: []
---
...
### `local_delegation_handler.py` / `delegation_orchestrator.py` (removed — historical)

This UserPromptSubmit-embedded delegation path no longer exists: neither file is present in
the current tree, and `docs/architecture/DELEGATION_ARCHITECTURE.md` documents the removal —
the auto-firing bridge was deleted, and its description below (5-gate pipeline: feature-flag
check, task classification, endpoint selection, LLM call, heuristic quality gate) is retained
here only as historical record of the design that ran before the removal. Delegation today runs
only on explicit invocation of the `/onex:delegate` skill, which dispatches
`node_delegate_skill_orchestrator` in omnimarket — it is not part of the UserPromptSubmit sync
...
| `src/omniclaude/hooks/lib/compliance_result_subscriber.py` | Background Kafka subscriber (moved from `plugins/onex/hooks/lib/`) |
| `local_delegation_handler.py` / `delegation_orchestrator.py` | Removed — see the note above; delegation now runs via the `/onex:delegate` skill, outside this sync path |
