...
| **Local** — `node_dod_verify` | the local Done gate / autoclose evidence sweep | the product repo's local `EvidenceCollector` |

Both read the same contract bytes. **For check types both runners execute, this
page states one shared behaviour** — that is the single-semantic-for-`test_passes` rule, and it is
...
```yaml
- check_type: command
  check_value: "gh pr checks 1719 --repo OmniNode-ai/<product-repo>"
```
...
(`${OMNI_HOME}/<product-repo>`) does not exist there and will be declined. That is
