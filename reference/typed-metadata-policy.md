...
status: current
date: "2026-08-25"
title: "Typed-Metadata Policy"
topics: [typed-metadata, pydantic, code-standards, onex-exclude]
refs: []
---

# Typed-Metadata Policy

> **Source**: onex_change_control `docs/policy/typed-metadata-policy.md`. Migrated to the
> knowledge base 2026-08-25 with the guard-script centralization status corrected against the
> live repo (see the migration note in §5).
...
2. The pre-commit hook (installed via the `check-no-untyped-metadata` entry point — see §5) scans for `dict[str, Any]` and `dict[str, object]` in Pydantic model files. Lines bearing `ONEX_EXCLUDE: dict_str_any` are skipped.
...
### Guard script — centralized, not a future path

> **2026-08-25 migration correction.** The source document described centralization as a
> "future centralization path," pending downstream repos declaring `onex_change_control` as a
> dev dependency. Verified live: `onex_change_control/pyproject.toml` already registers
> `check-no-untyped-metadata = "onex_change_control.scripts.check_no_untyped_metadata:main"`
> as an entry point, backed by `src/onex_change_control/scripts/check_no_untyped_metadata.py`
> (its own docstring: "Centralized in onex_change_control ... so downstream repos consume a
> single canonical copy"). `omniclaude` no longer carries a local
> `scripts/check_no_untyped_metadata.py` implementation — it consumes the entry point via its
> `onex_change_control` dependency (only a test file referencing the shared script remains in
> that repo). The centralization already happened.

The guard script `check-no-untyped-metadata` enforces this policy by scanning Pydantic model files for `dict[str, Any]` and `dict[str, object]`. Lines with `ONEX_EXCLUDE: dict_str_any` are exempt. Each downstream repo runs it as a pre-commit hook and/or CI check by declaring `onex_change_control` as a dependency and invoking the entry point directly — there is one canonical implementation, not a per-repo copy.
