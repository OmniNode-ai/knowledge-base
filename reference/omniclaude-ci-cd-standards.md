...
Triggers (corrected 2026-08-25 against live `.github/workflows/ci.yml`): `push` on `main` and
`hotfix/**`; `pull_request` on `main` and `dev`; `merge_group`; `workflow_dispatch`. There is no
`develop` branch anywhere in the live trigger config, and no plain `push` trigger for `dev`.
...
| `deploy-staging` | Deploy to Staging | After `build`; job `if:` gates on `github.ref == 'refs/heads/develop'` | **Structurally unreachable.** The workflow's own `push` trigger only fires on `main`/`hotfix/**` (no `develop`, no `dev`), so this job can never run as configured — verified against live `.github/workflows/ci.yml` 2026-08-25. |
| `deploy-production` | Deploy to Production | After `build`; `main` branch push only | Deploys to production environment (reachable — `main` is in the workflow's `push` trigger) |
