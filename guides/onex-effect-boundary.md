...
<!-- Migrated from omnibase_core:docs/guides/EFFECT_BOUNDARY_GUIDE.md on 2026-09-02 -->
...
def create_test_registry() -> ServiceEffectMockRegistry:
    ...
    registry.register_mock(
        ...
        lambda user_id: {
            "id": user_id,
            "name": "Test User",
            "email": "test@example.com",  # sanitization-ok: illustrative example value, not a real address
...
def test_fetch_user_mocked(enforcer):
    ...
    assert result["email"] == "test@example.com"  # sanitization-ok: illustrative example value, not a real address
...
- Replay Safety Integration Guide - Comprehensive replay safety system
- Effect Subcontract Guide - Declarative effect operations
- Threading Guide - Thread safety patterns
- Effect Node Tutorial - Building EFFECT nodes
