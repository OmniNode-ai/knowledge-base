...
status: current
date: "2026-09-02"
title: "ORCHESTRATOR Node Template"
topics:
  - omnibase-core
  - orchestrator
  - node
...
<!-- Migrated from omnibase_core:docs/guides/templates/ORCHESTRATOR_NODE_TEMPLATE.md on 2026-09-02 -->
...
class HandlerOrderFulfilled:
    ...
    async def handle(
        ...
        return ModelHandlerOutput.for_orchestrator(
            input_envelope_id=input_envelope_id,
            correlation_id=correlation_id,
            handler_id="handler-order-fulfilled",
            events=[
                {
...
output = ModelHandlerOutput.for_orchestrator(
    input_envelope_id=input_envelope_id,
    correlation_id=correlation_id,
    handler_id="handler-order-timeout",
    events=[{
