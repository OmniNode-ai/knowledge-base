...
<!-- Migrated from omnibase_core:docs/guides/templates/EFFECT_NODE_TEMPLATE.md on 2026-09-02 -->
...
class TestHandlerStorage:
    ...
    async def test_store_emits_event(self) -> None:
        ...
        result = await handler.handle_store(
            record={"user_id": "u1", "email": "a@b.com"},  # sanitization-ok: illustrative example value, not a real address
...
| Topic | Document |
|-------|----------|
| Node archetypes reference | Node Archetypes |
| Handler contract guide | Handler Contract Guide |
| Four-node architecture | ONEX Four-Node Architecture |
