...
<!-- Migrated from omnibase_core:docs/guides/node-building/03_COMPUTE_NODE_TUTORIAL.md on 2026-09-02 -->
...
**Prerequisites**: What is a Node?, Node Types
...
See Node Class Hierarchy Guide for detailed comparison.
...
> See Canonical Execution Shapes for the complete pattern.

**Key Constraint**: COMPUTE nodes must not perform any I/O operations (no network calls, no file writes, no database access). All I/O should be handled by EFFECT nodes. See Node Purity Guarantees for enforcement details.
...
- EFFECT Node Tutorial - Build a database interaction node
- REDUCER Node Tutorial - Build a data aggregation node
- ORCHESTRATOR Node Tutorial - Coordinate workflows
- Patterns Catalog - Reusable patterns
