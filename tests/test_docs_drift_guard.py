...
def test_real_manifest_is_inert_except_where_wave_2_has_landed_rows() -> None:
    ...
    # bucket A -> C and dropped to not-started 2026-08-26 (self-hoster's-book
    # scope ruling: application-migration-ledger.md, gateway-lane-deploy.md,
    # and the two managed-staging-canary runbooks all documented our own
    # cloud deployment topology or evidence-class content).
    ...
    # The 4 bucket-C reclassifications: restricted, cited-not-hosted, never
    # a live candidate for this public repo.
    restricted_not_started = [r for r in not_started if r["bucket"] == "C"]
    assert len(restricted_not_started) == 4
