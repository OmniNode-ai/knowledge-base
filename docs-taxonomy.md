...
- **Content migration has begun, one repo at a time.** As of 2026-08-25, this section was stale even before this edit — it named only the first migrated repository while four more had already landed. Corrected: five repositories now have manifest entries at repo-level `cutover_state: moved` with a per-document row for every Bucket-A candidate — `omnibase_core` (`docs/decisions/**` + `docs/standards/**` + `docs/troubleshooting/**`), `omniclaude` (`docs/architecture/**` + `docs/guides/**` + `docs/standards/**`), `omnimemory` (`docs/architecture/**` + `docs/runtime/**` + `docs/migrations/**`, every row already bumped to `cutover_state: pointer-live`, and — since an exhaustive per-file sweep of that repo landed — also `docs/*.md` and `docs/runbooks/**`, taking it to 13 rows (5 catalog, 5 migrated by the sweep, 3 deleted rather than migrated) and leaving the repo at README-plus-pointers), `omnibase_infra` (`docs/architecture/**` + `docs/guides/**` + `docs/runbooks/**`, most rows `pointer-live`), and `onex_change_control` (`docs/standards/**` + `docs/policy/**` + `docs/governance/**`, one bucket-B exception retained in-repo). Every other repository's manifest entry is still at `cutover_state: not-started`.
- **`omnibase_core` is fully migrated as of 2026-09-02** — the first repository to reach the bar. Its entire prose tree is gone: 113 pages landed here (40 `architecture/`, 51 `guides/`, 22 `reference/`), 37 routed to the private internal home under test 3, and 47 were deleted as pointer stubs or dated artifacts. Its `README.md` links here as the single documentation home. The line above naming that repo's scope as `docs/decisions/**` + `docs/standards/**` + `docs/troubleshooting/**` described the earlier partial wave and is superseded for that repo only.
- **A pointer stub is not a removal.** Earlier waves left a one-line "moved to the knowledge base" file behind in place of each migrated document. Those files are documentation that still exists in the product repo, and the repository README's link already carries the signposting role for the whole tree, so they are deleted rather than retained. 23 of `omnibase_core`'s 47 deletions were stubs left by its own earlier wave.
- **Regrowth enforcement is live in product repos, which the paragraph below predates.** A per-repo `kb-doc-gate` check now runs on pull requests across the fleet: `diff` mode fails a PR that adds or modifies markdown outside a small allowed set, and `strict` mode — which a repo turns on once its migration has landed, as `omnibase_core` now has — fails when any such file exists on the branch at all. The drift guard described next is a separate, manifest-driven mechanism and remains unwired.
